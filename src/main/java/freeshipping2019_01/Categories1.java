...
public class Categories1 {
     public static void main(String[] args) {
        ...
        driver.get("https://freeshipping2019.com/categories");
        ...
        for (int i = 0; i < textCat.size(); i++) {
            ...
            cat = driver.findElements(By.cssSelector("#storeAll > div:nth-child(3) > div:nth-child("+ indexCat +") > div:nth-child(2) > a"));
            // Trường hợp danh mục không có thì nhảy sang vòng for tiếp theo
            if (cat.size() == 0) {
                continue;
            }
            System.out.println("Nhóm " + textCat.get(i).toString() + " có: " + cat.size() + " categories");
            
            for (int j = 0; j < cat.size(); j++) {
                ((JavascriptExecutor) driver).executeScript("window.open()");
                ArrayList<String> tab = new ArrayList<String>(driver.getWindowHandles());
                driver.switchTo().window(tab.get(1));
                driver.get("https://freeshipping2019.com/categories#" + textCat.get(i).toString());
                String linkTextCat = driver.findElements(By.cssSelector("#storeAll > div:nth-child(3) > div:nth-child("+ indexCat +") > div:nth-child(2) > a")).get(j).getAttribute("href");
                driver.findElements(By.cssSelector("#storeAll > div:nth-child(3) > div:nth-child("+ indexCat +") > div:nth-child(2) > a")).get(j).click();
